...
class avoir_films_favoris {
	...
	public function avoir_acteurs_favoris_update() {
		...
		if (!empty($this->id_films)) {
			$query = $conn->prepare("UPDATE avoir_acteurs_favoris SET id_films = :id_films WHERE id_films = :id_films AND :id_users = id_users;");
			$query->execute(array("titre_films" => $this->titre_films, "id_films" => $this->id_films));
		}
		if (!empty($this->id_users)) {
			$query = $conn->prepare("UPDATE avoir_acteurs_favoris SET nom_films = :nom_films WHERE id_films = :id_films AND :id_users = id_users;");
